 ...
 * desc:
 */
class HotFragment : WaterFallGifsFragment(), LoadDataListener {
    ...
    override fun loadGifs(page: Int) {
        ...
        FetchHotResponse.getResponse(page, callback)
    }
    ...
    private fun handleFetchedFeeds(response: BaseResponse, isRefreshing: Boolean) {
        ...
        if (!ResponseHandler.handleResponse(response)) {
            val fetchHotFeeds = response as FetchHotResponse
            val status = fetchHotFeeds.status
            if (status == 0) {
                val feeds = fetchHotFeeds.gifs
                if (isRefreshing) {
                    activity.runOnUiThread {
                        feedList.clear()
